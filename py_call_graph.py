...
def record_call_graph(request):
    ...
    with open("../entry_point_extractor/commonEntryPoints/ycm.txt") as f:
        for line in f:
            temp = line.split("ycm")[-1][1:]
            temp = temp.replace("/", ".")
            temp = temp.replace(".py", ".*")
            temp = temp.replace("\n", "")
            if len(temp) > 0:
                total_input_files.add("ycm." + temp)
    with PyCallGraph(output=GraphvizOutput(dot_file='./dotFiles/%s.dot' % current_millis),
                     config=Config(debug=True,
                                   trace_filter=GlobbingFilter(
                                       ...
                                       exclude=['pycallgraph.*', '*.tests.*', '*.yml', "_pytest.*", "numpy.*"],
                                   )
                                   )):
        yield
